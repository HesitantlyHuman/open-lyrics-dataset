...
class DataPartitioner():
    def __init__(self, save_location = './data/', progress_file = './info/logging/progress.txt', partition_length = 8192):
        ...
        self._lyric_save_format = 'open-lyrics-{:3.}.csv'
        self._meta_save_format = 'open-lyrics-meta-{:3.}.csv'
    ...
    def save_collected_items(self):
        ...
        meta_save_path = os.path.join(self.lyrics_location, self._meta_save_format.format(self.current_file_number))
    ...
    def load_from_previous_if_exists(self, lyrics_location, metadata_location, progress_file):
        ...
        lyrics_file_names = [file_name for file_name in os.listdir(lyrics_location) if re.match(self._lyric_save_format, file_name)]
        meta_file_names = [file_name for file_name in os.listdir(metadata_location) if re.match(self._meta_save_format, file_name)]
        ...
        lyrics_number = int(re.match('[0-9]+', lyrics_file_names[-1])[0])
        meta_number = int(re.match('[0-9]+', meta_file_names[-1])[0])

        assert lyrics_number == meta_number, 'Lyrics and meta file numbers do not match!'

        loaded_lyrics_dataframe = pd.DataFrame(lyrics_file)
        loaded_meta_dataframe = pd.DataFrame(meta_file)
